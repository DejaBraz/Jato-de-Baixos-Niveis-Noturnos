#Dejanira F. Braz
setwd('C:/Users/Deja/Documents/Python Scripts/perfil/SON')

#dev.off()


climatologia_N <- read.table('Vene.SON.comjato.00-06.txt', sep='\t', head=TRUE)

climatologia_D <- read.table('Vene.SON.comjato.12-18.txt', sep='\t', head=TRUE)

jet_days_N <- read.table('BraNE.SON.semjato.00-06.txt', sep='\t', head=TRUE)

not_jet_days_N <- read.table('BraNE.SON.semjato.12-18.txt', sep='\t', head=TRUE)
...
plot(v_jet_N, n_jet_N, type='l', ylim=c(1000,500), xlim=c(1,13), xaxs = "i", yaxs = "i",  
     xlab='Wind Velocity (m/s)', ylab='Height (levels)', yaxp=c(1000,500,20), las=1, xaxp=c(1, 15,7), col="black", lwd = 3.5,font=2)
...
lines(v_not_jet_N, n_not_jet_N , type="l",col="black",lwd = 2.5, lty=2)
lines(v_clim_N, n_clim_N , type="l",col="firebrick2",lwd = 3.5)
lines(v_clim_D, n_clim_D , type="l",col="firebrick2",lwd = 2.5, lty=2)
lines(v_jet_N, n_jet_N , type="l",col="black",lwd = 2.5)

mtext("SON", col = 'red',cex =1.8, line=-2.0, side=3, adj = 0.02)




#legend('topright',
#       legend = c('Not LLJs day 06','Not LLJs day 12','LLJs day 00','LLJs day 12'),
